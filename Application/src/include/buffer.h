...
public:
  ...
  void setup(const std::vector<T> &data) {
    ...
    glBufferData(TYPE, sizeof(T) * data.size(), &data[0], GL_STATIC_DRAW);
    glBindBuffer(TYPE, 0);
  }
...
#pragma endregion
  ...
  void AddData(const std::vector<T> &data) {
    ...
    if (id == 0) {
      ...
      createBuffer();
      glBindBuffer(TYPE, id);
      glBufferData(TYPE, buffersize + data.size() * sizeof(T), nullptr, GL_STATIC_DRAW);
      glBufferSubData(TYPE, 0, buffersize, tempdata);
      glBufferSubData(TYPE, buffersize, sizeof(T) * data.size(), &data[0]);
      glBindBuffer(TYPE, 0);
    }
  }
