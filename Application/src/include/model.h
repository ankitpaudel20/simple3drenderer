...
static drawable<Vertex> processMesh(aiMesh *mesh, const aiScene *scene) {
  ...
  // 3. normal maps
  std::string normalMaps = loadMaterialTexture(material, aiTextureType_AMBIENT, "texture_normal");
  textures.push_back(normalMaps);
  // 4. height maps
  std::string heightMaps = loadMaterialTexture(material, aiTextureType_HEIGHT, "texture_height");
