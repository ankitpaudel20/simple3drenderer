...
namespace Model
{
    ...
    void processNode(aiNode *node, const aiScene *scene, std::vector<drawable<Vertex>>& meshes,int num=0) ;
...
// loads a model with supported ASSIMP extensions from file and stores the resulting meshes in the meshes vector.
    ...
    void processNode(aiNode *node, const aiScene *scene, std::vector<drawable<Vertex>>& meshes,int num) {
        int temp=num;
    ...
    drawable<Vertex> processMesh(aiMesh* mesh, const aiScene* scene)
	{
		// data to fill
...
//		textures.insert(textures.end(), heightMaps.begin(), heightMaps.end());
    ...
    // 1. diffuse maps
        ...
        textures.insert(textures.end(), specularMaps.begin(), specularMaps.end());
        // 3. normal maps
        std::vector<Texture> normalMaps = loadMaterialTexture(material, aiTextureType_AMBIENT, "texture_normal");
        textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
        // 4. height maps
		...
		Mesh toreturn= Mesh(vertices, indices, textures);
		return toreturn;
	}
	...
	Texture loadMaterialTexture(aiMaterial* mat, aiTextureType type, const std::string& typeName) {
        std::vector<Texture> textures;
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
            aiString str;
            mat->GetTexture(type, i, &str);
